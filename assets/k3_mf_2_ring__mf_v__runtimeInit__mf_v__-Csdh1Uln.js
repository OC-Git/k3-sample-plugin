...
function isReactNativeEnv() {
  var _navigator;
  return typeof navigator !== "undefined" && ((_navigator = navigator) == null ? undefined : _navigator.product) === "ReactNative";
}
...
function generateSnapshotFromManifest(manifest, options = {}) {
  ...
  if (!Object.keys(remotes).length) {
    ...
    remotesInfo = ((_manifest_remotes = manifest.remotes) == null ? undefined : _manifest_remotes.reduce((res2, next) => {
      let matchedVersion;
  ...
  let basicRemoteSnapshot = {
    ...
    remotesInfo,
    shared: manifest == null ? undefined : manifest.shared.map((item) => ({
      assets: item.assets,
      sharedName: item.name,
      version: item.version
    })),
    modules: exposes == null ? undefined : exposes.map((expose) => ({
      moduleName: expose.name,
      modulePath: expose.path,
      assets: expose.assets
    }))
  };
  if ((_manifest_metaData = manifest.metaData) == null ? undefined : _manifest_metaData.prefetchInterface) {
    const prefetchInterface = manifest.metaData.prefetchInterface;
    basicRemoteSnapshot = polyfills$1._({}, basicRemoteSnapshot, {
      prefetchInterface
    });
  }
  if ((_manifest_metaData1 = manifest.metaData) == null ? undefined : _manifest_metaData1.prefetchEntry) {
...
function createScript(info) {
  ...
  if (!script2) {
    ...
    script2.type = (attrs2 == null ? undefined : attrs2["type"]) === "module" ? "module" : "text/javascript";
  ...
  const onScriptComplete = async (prev, event) => {
    ...
    const onScriptCompleteCallback = () => {
      if ((event == null ? undefined : event.type) === "error") {
        (info == null ? undefined : info.onErrorCallback) && (info == null ? undefined : info.onErrorCallback(event));
      } else {
        (info == null ? undefined : info.cb) && (info == null ? undefined : info.cb());
      }
    };
    if (script2) {
...
const createScriptNode = typeof ENV_TARGET === "undefined" || ENV_TARGET !== "web" ? (url, cb, attrs, loaderHook) => {
  ...
  const getFetch = async () => {
    if (loaderHook == null ? undefined : loaderHook.fetch) {
  ...
  getFetch().then(async (f2) => {
    if ((attrs == null ? undefined : attrs["type"]) === "esm" || (attrs == null ? undefined : attrs["type"]) === "module") {
  ...
  return new Promise((resolve, reject) => {
    createScriptNode(url2, (error2, scriptContext2) => {
      if (error2) {
        ...
        const remoteEntryKey = (info == null ? undefined : (_info_attrs = info.attrs) == null ? undefined : _info_attrs["globalName"]) || `__FEDERATION_${info == null ? undefined : (_info_attrs1 = info.attrs) == null ? undefined : _info_attrs1["name"]}:custom__`;
...
const getTargetSnapshotInfoByModuleInfo = (moduleInfo, snapshot)=>{
    ...
    if ('version' in moduleInfo && moduleInfo['version']) {
        ...
        if ((getModuleInfoWithoutVersion == null ? undefined : getModuleInfoWithoutVersion.version) === version) {
...
function formatShare(shareArgs, from, name, shareStrategy) {
    ...
    return polyfills._extends({
        ...
        get,
        loaded: (shareArgs == null ? undefined : shareArgs.loaded) || 'lib' in shareArgs ? true : undefined,
...
function getTargetSharedOptions(options) {
    ...
    const resolver = (_extraOptions_resolver = extraOptions == null ? undefined : extraOptions.resolver) != null ? _extraOptions_resolver : defaultResolver;
    return Object.assign({}, resolver(shareInfos[pkgName]), extraOptions == null ? undefined : extraOptions.customShareInfo);
}
...
function registerPlugins$1(plugins, instance) {
    ...
    if (globalPlugins.length > 0) {
        globalPlugins.forEach((plugin)=>{
            if (plugins == null ? undefined : plugins.find((item)=>item.name !== plugin.name)) {
                plugins.push(plugin);
            }
        });
    }
...
useLinkPreload = true) {
    ...
    if (host.options.inBrowser) {
        ...
        if (useLinkPreload) {
            ...
            const defaultAttrs = {
                fetchpriority: 'high',
                type: (remoteInfo == null ? undefined : remoteInfo.type) === 'module' ? 'module' : 'text/javascript'
            };
            jsAssetsWithoutEntry.forEach((jsUrl)=>{
...
let Module = class Module {
    ...
    async get(id, expose, options, remoteSnapshot) {
        ...
        if (!this.inited) {
            ...
            if (typeof (remoteEntryExports == null ? undefined : remoteEntryExports.init) === 'undefined') {
...
class PluginSystem {
    applyPlugin(plugin, instance) {
        ...
        if (!this.registerPlugins[pluginName]) {
            ...
            plugin.apply == null ? undefined : plugin.apply.call(plugin, instance);
...
function generatePreloadAssets(origin, preloadOptions, remote, globalSnapshot, remoteSnapshot) {
    ...
    traverseModuleInfo(globalSnapshot, remote, (moduleInfoSnapshot, remoteInfo, isRoot)=>{
        ...
        if (normalizedPreloadExposes.length && 'modules' in moduleInfoSnapshot) {
            ...
            moduleAssetsInfo = moduleInfoSnapshot == null ? undefined : (_moduleInfoSnapshot_modules = moduleInfoSnapshot.modules) == null ? undefined : _moduleInfoSnapshot_modules.reduce((assets, moduleAssetInfo)=>{
                if ((normalizedPreloadExposes == null ? undefined : normalizedPreloadExposes.indexOf(moduleAssetInfo.moduleName)) !== -1) {
    ...
    if (remoteSnapshot.shared && remoteSnapshot.shared.length > 0) {
        ...
        remoteSnapshot.shared.forEach((shared)=>{
            var _options_shared;
            const shareInfos = (_options_shared = options.shared) == null ? undefined : _options_shared[shared.sharedName];
            if (!shareInfos) {
                return;
            }
    ...
    async loadRemoteSnapshotInfo({ moduleInfo, id, expose }) {
        ...
        if (hostSnapshot && 'remotesInfo' in hostSnapshot && !getInfoWithoutType(hostSnapshot.remotesInfo, moduleInfo.name).value) {
            if ('version' in moduleInfo || 'entry' in moduleInfo) {
                hostSnapshot.remotesInfo = polyfills._extends({}, hostSnapshot == null ? undefined : hostSnapshot.remotesInfo, {
                    [moduleInfo.name]: {
...
class SharedHandler {
    ...
    async loadShare(pkgName, extraOptions) {
        ...
        if (shareInfo == null ? undefined : shareInfo.scope) {
        ...
        if (registeredShared && registeredShared.lib) {
            ...
            if (extraOptions == null ? undefined : extraOptions.customShareInfo) {
                return false;
            }
            const asyncLoadProcess = async ()=>{
    ...
    initializeSharing(shareScopeName = DEFAULT_SCOPE, extraOptions) {
        const { host } = this;
        const from = extraOptions == null ? undefined : extraOptions.from;
        const strategy = extraOptions == null ? undefined : extraOptions.strategy;
        let initScope = extraOptions == null ? undefined : extraOptions.initScope;
        const promises = [];
        if (from !== 'build') {
        ...
        const register = (name, shared)=>{
            ...
            const activeVersionEager = Boolean(activeVersion && (activeVersion.eager || ((_activeVersion_shareConfig = activeVersion.shareConfig) == null ? undefined : _activeVersion_shareConfig.eager)));
    ...
    loadShareSync(pkgName, extraOptions) {
        ...
        if (shareInfo.get) {
            ...
            if (module instanceof Promise) {
                const errorCode = (extraOptions == null ? undefined : extraOptions.from) === 'build' ? errorCodes$1.RUNTIME_005 : errorCodes$1.RUNTIME_006;
...
class RemoteHandler {
    ...
    registerRemotes(remotes, options) {
        ...
        remotes.forEach((remote)=>{
            this.registerRemote(remote, host.options.remotes, {
                force: options == null ? undefined : options.force
            });
        });
    }
    ...
    registerRemote(remote, targetRemotes, options) {
        ...
        const normalizeRemote = ()=>{
            if (remote.alias) {
                ...
                const findEqual = targetRemotes.find((item)=>{
                    var _item_alias;
                    return remote.alias && (item.name.startsWith(remote.alias) || ((_item_alias = item.alias) == null ? undefined : _item_alias.startsWith(remote.alias)));
                });
        ...
        if (!registeredRemote) {
            ...
            if (options == null ? undefined : options.force) {
...
let initResolve, initReject;
    ...
    var k3_mf_2_ring__mf_v__runtimeInit__mf_v__ = {
      initPromise,
      initResolve,
      initReject
    };

export { index_cjs$3 as a, init_1 as i, k3_mf_2_ring__mf_v__runtimeInit__mf_v__ as k };
